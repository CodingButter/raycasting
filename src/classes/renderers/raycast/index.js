...
export default class RayCast {
  ...
  async addImages() {
    const gold = await ImageLoader.loadImage(texture)
    this.__floorTexture = new Texture(gold)
    this.__texture = new Texture(gold)
    this.__loaded = true
  }
  attachCamera(camera) {
  ...
  render(ctx) {
    ...
    for (var column = 0; column < this.__rays.length; column++) {
      ...
      while (y < this.__height) {
        ...
        const beta = Math.abs(ray.angle - this.__camera.angle)
        const worldDistFromPlayer = (projDist * this.__camera.height) / FloorProjectionRow
        const rowActualDistance = Math.cos(beta) * worldDistFromPlayer
        const pointX = this.__camera.position.x + Math.cos(ray.angle) * rowActualDistance
        const pointY = this.__camera.position.y + Math.sin(ray.angle) * rowActualDistance
        // ctx.save();
