...
export default class Ray {
  ...
  cast() {
    ...
    var yIntercept = Math.floor(this.__position.y / Map.TILE_SIZE) * Map.TILE_SIZE
    ...
    while (
      ...
      if (
        ...
        var horizontalWallHit = new Vector(nextHorizontalTouchX, nextHorizontalTouchY)
        break
      } else {
    ...
    var xIntercept = Math.floor(this.__position.x / Map.TILE_SIZE) * Map.TILE_SIZE
    ...
    if (verticalHitDistance < horizontalHitDistance) {
      ...
      if (this.__isRayFacingLeft) {
        this.__textureOffset =
          Map.TILE_SIZE -
          (this.__position.y - Math.floor(this.__position.y / Map.TILE_SIZE) * Map.TILE_SIZE)
      } else {
        this.__textureOffset =
          this.__position.y - Math.floor(this.__position.y / Map.TILE_SIZE) * Map.TILE_SIZE
      }
      ...
      if (this.__isRayFacingDown) {
        this.__textureOffset =
          Map.TILE_SIZE -
          (this.__position.x - Math.floor(this.__position.x / Map.TILE_SIZE) * Map.TILE_SIZE)
      } else {
        this.__textureOffset =
          this.__position.x - Math.floor(this.__position.x / Map.TILE_SIZE) * Map.TILE_SIZE
      }
